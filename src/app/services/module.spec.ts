...
describe('ModuleService', () => {
  ...
  describe('getModules (REQ-3)', () => {
    it('returns all modules on success', async () => {
      const mockModules = [
        { id: '1', title: 'Module 1', description: 'Desc 1', avatar: 'ava1', icon: 'icon1' },
        { id: '2', title: 'Module 2', description: 'Desc 2', avatar: 'ava2', icon: 'icon2' }
      ];
      fromSpy.select.and.returnValue(Promise.resolve({ data: mockModules, error: null }));

      const modules = await service.getModules();
      expect(modules).toEqual(mockModules as any);
