...
describe('TabContent', () => {
    ...
    beforeEach(async () => {
        ...
        mockSectionContentService = {
            ...
            evaluateLessonContent: jasmine.createSpy().and.returnValue(Promise.resolve({ aiFeedback: 'AI Feedback Content' })),
        };
    ...
    it('enables AI evaluation only when markdown content exists and conditions are met', () => {
        expect(component.hasMarkdownContent()).toBeFalse();
        expect(component.canEvaluate()).toBeFalse();

        // Add a non-markdown content block
        component.addContent(SectionContentType.VIDEO);
        expect(component.hasMarkdownContent()).toBeFalse();
        expect(component.canEvaluate()).toBeFalse();
        ...
        component.addContent(SectionContentType.MARKDOWN);
        expect(component.hasMarkdownContent()).toBeTrue();
        expect(component.canEvaluate()).toBeTrue();

        // Disable when saving or evaluating
        ...
        expect(component.canEvaluate()).toBeFalse();
        component.isEvaluating.set(false);
        expect(component.canEvaluate()).toBeTrue();
    });

    it('submits markdown contents and handles AI evaluation response successfully', async () => {
        component.addContent(SectionContentType.MARKDOWN);
        component.updateContent(0, { content: 'Markdown Content Block 1' });
        
        component.addContent(SectionContentType.MARKDOWN);
        component.updateContent(1, { content: 'Markdown Content Block 2' });

        component.lessonForm.patchValue({
            title: 'Test Lesson Title',
            description: 'Test Lesson Description details here.'
        });

        await component.evaluateContent();

        expect(mockSectionContentService.evaluateLessonContent).toHaveBeenCalledWith(
            'Test Lesson Title',
            'Test Lesson Description details here.',
            ['Markdown Content Block 1', 'Markdown Content Block 2']
        );
        expect(component.aiFeedback()).toBe('AI Feedback Content');
        expect(component.errorMessage()).toBeNull();
        expect(component.isEvaluating()).toBeFalse();
    });

    it('handles AI evaluation errors and displays error messages', async () => {
        component.addContent(SectionContentType.MARKDOWN);
        component.updateContent(0, { content: 'Markdown Content Block' });

        mockSectionContentService.evaluateLessonContent.and.returnValue(Promise.reject(new Error('AI Evaluation Failed')));

        await component.evaluateContent();

        expect(component.aiFeedback()).toBeNull();
        expect(component.errorMessage()).toBe('AI Evaluation Failed');
        expect(component.isEvaluating()).toBeFalse();
    });
});
